const hg = new ProgramApi();

/**
 * @description The global entry point for accessing HomeGenie's core API.
 *              Typically exposed as the 'hg' object in the scripting environment.
 */
class ProgramApi {
  /**
   * Modules Helper. Manages and provides access to HomeGenie's home automation modules (devices).
   */
  ...
  /**
   * Events Helper. Provides event subscription methods, often for module events or program state changes.
   */
  when: EventsHelper;
  /**
   * API Helper. Provides general access to HomeGenie's public API endpoints, often for calling service methods.
   */
  api: ApiHelper;
  /**
   * Data Helper. Provides methods for accessing program-specific data storage,
   * LiteDB embedded database functionality, and system backup management.
   */
  data: DataHelper;
  // Miscellaneous functions
  /**
   * Pauses the current program execution for the given amount of seconds.
   * **Note:** This is typically a blocking operation in the scripting environment.
   * @param seconds The duration of the pause in seconds.
   */
  pause(seconds: number);
  /**
   * Delays the current program execution for the given amount of seconds. (Alias of `pause(...)`).
   * **Note:** This is typically a blocking operation in the scripting environment.
...
/**
 * @description Defines the interface for event monitoring and handling within the HomeGenie automation environment.
 *              Typically exposed as `hg.on` or `hg.when` in the scripting environment.
 */
class EventsHelper {
  /**
   * Calls the specified handler after HomeGenie service has started.
   * @param handler The handler function to call. Returning `false` prevents the event from being routed to other listeners.
   * @example
   * hg.when.SystemStarted( () => {
   *   hg.program.Say("HomeGenie is now ready!");
   *   return false;
   * });
   * @returns The EventsHelper instance for method chaining.
   */
  systemStarted(handler: () => boolean): EventsHelper;

  /**
   * Calls the specified handler when HomeGenie service is stopping.
   * @param handler The handler function to call. Returning `false` prevents the event from being routed to other listeners.
   * @example
   * hg.when.SystemStopping( () => {
   *   hg.program.Say("See ya soon!");
   *   return true;
   * });
   * @returns The EventsHelper instance for method chaining.
   */
  systemStopping(handler: () => boolean): EventsHelper;

  /**
   * Calls the specified handler when the program (script) is being stopped.
   * @param handler The handler function to call. Returning `false` prevents the event from being routed to other listeners.
   * @example
   * hg.when.ProgramStopping( () => {
   *   hg.program.Say("Oh-oh! I'm quitting!");
   *   return true;
   * });
   * @returns The EventsHelper instance for method chaining.
   */
  programStopping(handler: () => boolean): EventsHelper;

  /**
   * Calls the specified handler function when a parameter of a module changed (i.e., the change has been confirmed).
   * If the handler returns `false`, event propagation will stop.
   * @param handler The handler function to call.
   * @returns The EventsHelper instance for method chaining.
   * @see ModuleParameterIsChanging
   */
  moduleParameterChanged(handler: (module: ModuleHelper, parameter: ModuleParameter) => boolean): EventsHelper;

  /**
   * Calls the specified handler function when a parameter of a module is changing (i.e., before the change is confirmed).
   * If the handler returns `false`, event propagation will stop.
   * @param handler The handler function to call.
   * @returns The EventsHelper instance for method chaining.
   * @see ModuleParameterChanged
   */
  moduleParameterIsChanging(handler: (module: ModuleHelper, parameter: ModuleParameter) => boolean): EventsHelper;

  /**
   * Registers a handler for a received web service call (API call), expecting a JSON object as a result.
   * @param apiCall The specific API call path (e.g., 'Automation/Program/MyProgram/MyApi').
   * @param handler The handler function that receives the request data and returns a JSON object result.
   * @returns The EventsHelper instance for method chaining.
   */
  webServiceCallReceived(apiCall: string, handler: (requestData: any) => any): EventsHelper;

  /**
   * Registers a handler for a received web service call (API call), expecting a string as a result.
   * @param apiCall The specific API call path (e.g., 'Automation/Program/MyProgram/MyApi').
   * @param handler The handler function that receives the request data and returns a string result.
   * @returns The EventsHelper instance for method chaining.
   */
  webServiceCallReceived(apiCall: string, handler: (requestData: any) => string): EventsHelper;
}
/**
...
class ApiHelper {
   ...
   * @param apiCall The base API call path to handle (e.g., 'HomeAutomation.PhilipsHue').
   * @param handler The handler function that receives the request arguments and returns the response object (or string).
   * @returns The ApiHelper instance for method chaining.
   */
  handle(apiCall: string, handler: (args: any) => any): ApiHelper;

  /**
   * Defines a handler function that will be called when a web service call starting with `apiCall` is received,
   * expecting a string response.
   * @param apiCall The base API call path to handle (e.g., 'HomeAutomation.PhilipsHue').
   * @param handler The handler function that receives the request arguments and returns a string response.
   * @returns The ApiHelper instance for method chaining.
   */
  handle(apiCall: string, handler: (args: any) => string): ApiHelper;


  /**
   * Parses the given API call (string or command object) as a `MigInterfaceCommand` object.
   * This object contains structured fields like `Domain`, `Address`, `Command`, and `Data`.
   * @param apiCall API Command instance (MigInterfaceCommand) or string (e.g., "HomeAutomation.X10/A5/Control.Level/50").
   * @returns The parsed MigInterfaceCommand object.
   */
  parse(apiCall: string | any): MigInterfaceCommand;
...
/**
 * @description Represents a parsed command received by the HomeGenie/MIG API.
 *              This object is typically the result of `hg.api.parse()` and is used
 *              to read structured API calls.
 */
class MigInterfaceCommand {

  /**
   * Gets or sets the domain of the command (the first part of the API call, e.g., 'HomeAutomation.ZWave').
   */
  domain: string;

  /**
   * Gets or sets the address of the module/entity targeted by the command.
   */
  address: string;

  /**
   * Gets the specific command requested (e.g., 'Control.Level', 'Status.Level').
   */
  command: string;

  /**
   * Gets or sets any supplementary data object passed with the request (e.g., in a POST body).
   */
  data: any;

  /**
   * Gets the full unparsed original request string (e.g., "HomeAutomation.X10/A5/Control.Level/50").
   */
  originalRequest: string;

  /**
   * Gets the command options list as a slash-separated string.
   * @returns The slash-separated string of all options.
   */
  get optionsString(): string;

  /**
   * Retrieves an option value from the command request based on its index.
   * The options are the path segments following the command (index starts at 0).
   * @param index The zero-based index of the option to retrieve.
   * @returns The URL-unescaped option value as a string.
   */
...
/**
 * @description Module Helper class. This class is a module instance wrapper and is used as the return value of ModulesManager.Get() method.
 *              It allows easy access and manipulation of a single module's data and state.
 */
class ModuleHelper extends ModulesManager {

  /**
   * Determines whether this module has the given name.
   * @param name The name to check against the module's name.
   * @returns `true` if this module has the given name; otherwise, `false`.
   */
  is(name: string): boolean;

  /**
   * Gets a value indicating whether this ModuleHelper has a valid module instance loaded.
   * @returns `true` if a module instance is valid; otherwise, `false`.
   */
  exists: boolean;

  /**
   * Determines whether this module belongs to the specified domain.
   * @param domain The domain name to check (e.g., 'HomeAutomation.ZWave').
   * @returns `true` if this module belongs to the specified domain; otherwise, `false`.
   */
  isInDomain(domain: string): boolean;

  /**
   * Gets the underlying module instance object.
   * @returns The Module object instance.
   */
  instance: Module;

  /**
   * Determines whether this module is in the specified groupList.
   * @param groupList Comma separated group names (e.g., 'LivingRoom,Kitchen').
   * @returns `true` if this module is in any of the specified groups; otherwise, `false`.
   */
  isInGroup(groupList: string): boolean;

  /**
   * Determines whether this module is of one of the types specified in typeList.
   * @param typeList Comma separated type list (e.g., 'Switch,Light').
   * @returns `true` if this module is of one of the device types specified; otherwise, `false`.
   */
  isOfDeviceType(typeList: string): boolean;

  /**
   * Determines whether this module has the specified feature active.
   * @param feature The feature name to check (e.g., 'Control.Level').
   * @returns `true` if this module has the specified feature active; otherwise, `false`.
   */
  hasFeature(feature: string): boolean;

  /**
   * Determines whether this module has the specified parameter.
   * @param parameter The parameter name (e.g., 'Status.Level').
   * @returns `true` if this module has the specified parameter; otherwise, `false`.
   */
   ...
   * @param parameter The parameter name (e.g., 'Status.Level').
   * @returns The ModuleParameter object.
   */
  parameter(parameter: string): ModuleParameter;

  /**
   * Sets the value of a parameter and emits the associated event.
   * @param parameter Parameter name (e.g., 'Control.Level').
   * @param value The value to set (e.g., '50').
   * @returns The ModulesManager instance (inherited) for method chaining.
   */
  setParameter(parameter: string, value: string): ModulesManager;
  ...
  /**
   * Emits a new parameter value for this module.
   * This is the primary method to update a module parameter and trigger event propagation.
   * @param parameter Parameter name.
   * @param value The new parameter value to set.
   * @param description Event description (optional).
   * @returns The ModuleHelper instance for method chaining.
   */
  emit(parameter: string, value: any, description?: string): ModuleHelper;
