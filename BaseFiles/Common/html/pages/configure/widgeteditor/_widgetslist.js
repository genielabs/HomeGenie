HG.WebApp.WidgetsList = HG.WebApp.WidgetsList || new function () { var $$ = this;

    $$.PageId = 'page_configure_widgetlist';
    $$._currentWidget = '';
    ...
    $$.RefreshList = function (items) {
        ...
        for (var i = 0; i < items.length; i++) {
            $$.GetWidgetIcon(items[i], i, function (icon, id) {
                listMenu.find('li a img').eq(id).attr('src', icon);
            });
        }
        listMenu.find('li').on('click', function () {
            $$._currentWidget = $(this).attr('data-item-name');
        });
    };

    $$.GetWidgetIcon = function (widget, elid, callback) {
